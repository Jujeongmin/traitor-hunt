...
export function fightStats(c: { xp: number; gear: Gear; job: JobId | null }): { maxHp: number; gear: FightBonus } {
  return fightStatsAt(levelOf(c.xp).level, c.gear, c.job);
}

export function fightStatsAt(level: number, gear: Gear, job: JobId | null): { maxHp: number; gear: FightBonus } {
...
export function combatPower(c: { xp: number; playerClass: PlayerClass; gear: Gear; job: JobId | null }): number {
  return combatPowerAt(levelOf(c.xp).level, c.playerClass, c.gear, c.job);
}

export function combatPowerAt(level: number, playerClass: PlayerClass, gear: Gear, job: JobId | null): number {
  const { maxHp, gear: bonus } = fightStatsAt(level, gear, job);
